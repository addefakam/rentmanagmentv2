...
// (default: the woreda registrar; privileged acts pass the specific officer).
...
async function main() {
  ...
  const stamped = await api(`/api/registration-files/${file.id}`, "PATCH", { action: "stamp" }, "STF-0002");
  ...
  try {
    adj = await api("/api/adjustments", "POST", { year: adjYear, percentage: 8, basisStudy: "Bureau annual market study" }, "STF-0004");
    step(`adjustment ${adjYear} drafted (+${adj.percentage}%)`);
  } catch {
    const all = await api("/api/adjustments", "GET");
    adj = all.find((a: { year: number }) => a.year === adjYear) ?? null;
    if (!adj) throw new Error(`adjustment ${adjYear} could not be drafted or found`);
    step(`adjustment ${adjYear} reused (${adj.status})`);
  }
  if (adj.status === "DRAFT") {
    const published = await api("/api/adjustments", "PATCH", { id: adj.id, action: "publish" }, "STF-0005");
    step(`published June 1 (${published.status}) — pre-effect check window opened`);
    adj = published;
  }
  if (adj.status === "PUBLISHED") {
    const effected = await api("/api/adjustments", "PATCH", { id: adj.id, action: "effect" }, "STF-0005");
    step(`effective June 30 (${effected.status}) — amendment window opened`);
    adj = effected;
  }
  const vCheck = await api("/api/adjustments", "POST", { action: "validate-increase", fileId: registered.file.id, proposedRent: 12500 }, "STF-0001");
  ...
  const overCheck = await api("/api/adjustments", "POST", { action: "validate-increase", fileId: registered.file.id, proposedRent: 14000 }, "STF-0001");
  ...
  await api("/api/appeals", "PATCH", { id: appeal.id, action: "schedule" }, "STF-0006");
  await api("/api/appeals", "PATCH", { id: appeal.id, action: "hear" }, "STF-0006");
  const appealDecided = await api("/api/appeals", "PATCH", { id: appeal.id, action: "decide" }, "STF-0006");
  step(`committee decided (${appealDecided.status})`);
  const sweep = await api("/api/deadlines", "POST", undefined, "STF-0008");
  ...
  const team = allTeams.teams.find((t: { teamCode: string }) => t.teamCode === "CT-BOLE-01")
    ?? await api("/api/control", "POST", { kind: "team", teamCode: "CT-BOLE-01", subCityId: woreda.parentId, members: "Inspector One, Inspector Two" }, "STF-0003");
  ...
  const notified = await api("/api/penalties", "PATCH", { id: cashCase.id, action: "notify" }, "STF-0005");
  const paid = await api("/api/penalties", "PATCH", { id: cashCase.id, action: "pay" }, "STF-0005");
  step(`cash penalty ${paid.status} (${notified.computedAmount} ETB = 10%)`);
  const referred = await api("/api/penalties", "PATCH", { id: vac.id, action: "refer", targetBody: "TAX_AUTHORITY" }, "STF-0005");
  ...
  const backup = await api("/api/backups", "POST", { environmentId: env.id, type: "FULL", location: "PROD vault" }, "STF-0008");
  step(`backup on ${env.stage} succeeded`, () => { if (backup.status !== "SUCCEEDED") throw new Error("backup not succeeded"); });
  const snapshot = await api("/api/analytics", "POST", { orgUnitId: bureau.id, period: "2026-09" }, "STF-0004");
