...
afterAll(async () => {
  // Leave the platform in the shipped state: demo read surface, Wave 1 READY,
  // checklist green, O-7 closed.
  await restoreDemoMode();
  const wave1 = await prisma.goLiveWave.findUnique({ where: { code: "WAVE-1" } });
  if (wave1 && wave1.status !== "READY") {
...
describe("Phase 8 seed state: platform ships go-live READY", () => {
  ...
  it("seeded the four-wave rollout with the pilot live under the G7 authorization", async () => {
    ...
    expect(waves[1].status).toBe("READY"); // checklist green, awaiting the G8 order
    expect(waves[2].status).toBe("PLANNED");
  ...
  it("signed the hypercare schedule and staffed the escalation tree", async () => {
    ...
    expect(plan?.status).toBe("SIGNED");
...
describe("Phase 8 wave cutover and the go-live order (Gate G8)", () => {
  it("re-executes the checklist idempotently and stays green", async () => {
  ...
  it("blocks the go-live order while any cutover item is not GREEN, then executes on green", async () => {
    // Block: force one item RED (drill only; restored below).
    const item = await prisma.cutoverItem.findUnique({ where: { waveCode_seq: { waveCode: "WAVE-1", seq: 8 } } });
    const original = { status: item!.status, evidence: item!.evidence };
    await prisma.cutoverItem.update({ where: { id: item!.id }, data: { status: "RED" } });
    try {
      await giveGoLiveOrder("WAVE-1", "DRILL-GOLIVE-01", { staffCode: "STF-0005" });
    ...
    // Execute the drill order on green.
    const ordered = await giveGoLiveOrder("WAVE-1", "DRILL-GOLIVE-01", { staffCode: "STF-0005" });
    expect(ordered.status).toBe("LIVE");
    expect(ordered.goLiveOrderRef).toBe("DRILL-GOLIVE-01");
    expect(ordered.cutoverAt).not.toBeNull();

    // Restore the shipped state: Wave 1 READY, awaiting the real G8 order.
    await prisma.goLiveWave.update({
      where: { code: "WAVE-1" },
      data: { status: "READY", goLiveOrderRef: null, cutoverAt: null },
    });
  ...
  it("refuses a go-live order on the replication-preparation wave", async () => {
    try {
      await giveGoLiveOrder("WAVE-3", "DRILL-GOLIVE-03", { staffCode: "STF-0005" });
